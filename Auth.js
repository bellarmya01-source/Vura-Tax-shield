...
// Only saves if user is logged in — silently skips
// if not logged in (no alert, no crash)
async function saveCalculationToHistory(
    ...
    try {
        ...
        if (!user) {
            console.log('Not logged in — history not saved.')
            return
        }
...
async function getUserCalculationHistory() {
    try {
        const user = await getCurrentUser()
        if (!user) {
            console.log('Not logged in — cannot fetch history.')
            return []
        }
        ...
        container.innerHTML = `
            <div style="text-align:center;padding:40px 20px;color:#627d98;">
                ...
                <p style="font-size:0.9rem;">
                    Calculations are saved per account.
                    <a href="index.html"
